...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            {
                animalID = ourAnimals[i, 0].Substring(6);
                animalSpecies = ourAnimals[i, 1].Substring(9);
                ...
                {
                    if (!int.TryParse(animalAge, out _))
                    {
                        do
                        {
                    ...
                    if (string.IsNullOrEmpty(animalPhysicalDescription) || animalPhysicalDescription == "tbd")
                    {
                        do
                        {
                            Console.WriteLine($"Enter a physical description for ID #: {animalID} (size, color, breed, gender, weight, housebroken)");
                            readResult = Console.ReadLine();
                            if (readResult != null)
                            {
                                animalPhysicalDescription = readResult.ToLower();
        ...
        case "4":
            ...
            for (int i = 0; i < petCount; i++)
            {
                ...
                {
                    if (string.IsNullOrEmpty(animalNickname) || animalNickname == "tbd")
                    {
                        do
                        {
                            Console.WriteLine($"Enter a nickname for ID #: {animalID}");
                            readResult = Console.ReadLine();
                            if (readResult != null)
                            {
                                animalNickname = readResult.ToLower();
                    ...
                    if (string.IsNullOrEmpty(animalPersonalityDescription) || animalPersonalityDescription == "tbd")
                    {
                        do
                        {
                            Console.WriteLine($"Enter a personality description for ID #: {animalID} (likes or dislikes, tricks, energy level)");
                            readResult = Console.ReadLine();
                            if (readResult != null)
                            {
                                animalPersonalityDescription = readResult.ToLower();
